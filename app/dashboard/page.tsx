...
function DashboardContent() {
    ...
    const [profileCommentsModal, setProfileCommentsModal] = useState<{ profile: any; comments: any[] } | null>(null);
            ...
            if (data.success) {
                ...
                if (prevTasksRef.current.length > 0) {
                    for (const nt of newTasks) {
                        ...
                        const cmdName = nt.command === 'post_to_linkedin' ? 'Post to LinkedIn' : nt.command === 'scrape_feed_now' ? 'Quick Capture' : nt.command === 'scrape_profile' ? 'Scrape Profile' : nt.command === 'linkedin_get_feed_api' ? 'Capture Feed' : nt.command === 'linkedin_search_posts_api' ? 'Search Posts' : nt.command === 'linkedin_get_trending_api' ? 'Trending' : nt.command;
                        if (!prev) {
                            ...
                            else if (nt.status === 'failed' || nt.status === 'cancelled') {
                                const reason = nt.data?.error || nt.error || '';
                                addTaskNotification(reason ? `Failed: ${cmdName} - ${reason}` : `Failed: ${cmdName}`, 'error');
                            }
    ...
    // Poll tasks frequently for responsive UI — 5s when tasks are active, 15s otherwise
    ...
    useEffect(() => {
        ...
        const taskInterval = setInterval(() => {
            ...
            loadTasks(true);
            // Also refresh connectivity more often when tasks are active
            if (hasActiveTasks) checkExtensionConnectivity();
        }, 5000); // 5s polling for fast task status updates
        const heartbeatInterval = setInterval(() => checkExtensionConnectivity(), 15000);
    ...
    useEffect(() => {
        const updateCountdown = () => {
            ...
            // Handle pending countdown - show elapsed time waiting for extension
            if (pendingTasks.length > 0) {
                ...
                if (oldestPending && oldestPending.createdAt) {
                    ...
                    const countdownText = elapsed < 3 ? 'Starting soon...' : `Waiting for extension... ${elapsed}s`;
    ...
    useEffect(() => {
        ...
        const checkPendingTasks = () => {
            ...
            const inProgressTasks = tasks.filter(t => t.status === 'in_progress');
            const now = Date.now();
            // Timeouts: 45s for pending (extension polls every 15s, dashboard polls every 5s)
            // 10 minutes for in_progress tasks (some tasks like feed scraping take several minutes)
            const PENDING_TIMEOUT_MS = 45000; // 45 seconds - 3 extension poll cycles
            const IN_PROGRESS_TIMEOUT_MS = 600000; // 10 minutes for in_progress tasks

            // Check if extension is TRULY active (heartbeat within last 60 seconds - extension sends every 15s)
            const isExtensionTrulyActive = extensionLastSeen && (now - extensionLastSeen.getTime()) < 60000;
            ...
            // Show "Starting soon..." notification only briefly (first 5 seconds)
            // After that, the pendingCountdown shows "Waiting for extension..." which is more accurate
            if (pendingTasks.length > 0) {
                const oldestPending = pendingTasks.reduce((oldest, task) => {
                    const taskTime = new Date(task.createdAt).getTime();
                    const oldestTime = oldest ? new Date(oldest.createdAt).getTime() : Infinity;
                ...
                if (oldestPending && oldestPending.createdAt) {
                    ...
                    // Show notification for the first 5 seconds, then let countdown handle it
                    setShowStartingSoonNotification(elapsed < 5000);
                }
            } else {
            ...
            // For pending tasks: fail ONLY if extension is completely disconnected AND timeout exceeded
            // When extension is connected, NEVER auto-fail — trust the extension to pick up the task
                ...
                pendingTasks.forEach(task => {
                    if (task.createdAt && !pendingTaskTimeoutsRef.current.has(task.id)) {
                        ...
                        if (elapsed > PENDING_TIMEOUT_MS) {
                            // Double-check connectivity before failing - re-fetch heartbeat status
                            const timeoutId = setTimeout(async () => {
                                const token = localStorage.getItem('authToken');
                                if (!token) return;

                                // Re-check extension connectivity before marking as failed
                                try {
                                    const hbRes = await fetch('/api/extension/heartbeat', { headers: { 'Authorization': `Bearer ${token}` } });
                                    const hbData = await hbRes.json();
                                    if (hbData.connected) {
                                        // Extension came back online - don't fail the task
                                        pendingTaskTimeoutsRef.current.delete(task.id);
                                        return;
                                    }
                                } catch (e) { /* proceed with failing */ }
                                ...
                                try {
                                    ...
                                    const reason = 'Extension disconnected';
                                    const suggestion = 'Connect your extension to start processing tasks.';

                                    const cmdLabel = task.command === 'post_to_linkedin' ? 'Post to LinkedIn' :
                                        task.command === 'scrape_feed_now' ? 'Quick Capture' :
                                            task.command === 'scrape_profile' ? 'Scrape Profile' :
                                                task.command === 'linkedin_get_feed_api' ? 'Capture Feed' :
                                                    task.command === 'linkedin_search_posts_api' ? 'Search Posts' :
                                                        task.command === 'linkedin_get_trending_api' ? 'Trending' :
            ...
            // For in_progress tasks: only fail if extension is completely disconnected AND task stalled for timeout period
            inProgressTasks.forEach(task => {
                if (task.createdAt && !inProgressTaskTimeoutsRef.current.has(task.id)) {
                    ...
                    // Only fail if ALL conditions met:
                    // 1. Total elapsed time exceeds timeout (10 minutes)
                    // 2. No recent activity for the full timeout period
                    // 3. Extension is NOT connected at all
                    if (elapsed > IN_PROGRESS_TIMEOUT_MS && timeSinceLastActivity > IN_PROGRESS_TIMEOUT_MS && !extensionConnected) {
                        const timeoutId = setTimeout(async () => {
                            const token = localStorage.getItem('authToken');
                            if (!token) return;

                            // Re-check extension connectivity before failing
                            try {
                                const hbRes = await fetch('/api/extension/heartbeat', { headers: { 'Authorization': `Bearer ${token}` } });
                                const hbData = await hbRes.json();
                                if (hbData.connected) {
                            ...
                            try {
                                ...
                                const reason = 'Extension disconnected';
                                const suggestion = 'Connect your extension to resume task processing.';
    ...
    return (
        <div data-theme={theme} style={{
                                        ...
                                        {(task.status === 'pending' || task.status === 'in_progress') && (
                                            <button onClick={async (e) => {
                                                ...
                                                try {
                                                    ...
                                                    addTaskNotification(`Stopped: ${task.command === 'post_to_linkedin' ? 'Post to LinkedIn' : task.command === 'scrape_feed_now' ? 'Quick Capture' : task.command === 'scrape_profile' ? 'Scrape Profile' : task.command === 'linkedin_get_feed_api' ? 'Capture Feed' : task.command === 'linkedin_search_posts_api' ? 'Search Posts' : task.command === 'linkedin_get_trending_api' ? 'Trending' : task.command === 'start_bulk_commenting' ? 'Bulk Commenting' : task.command === 'start_import_automation' ? 'Import Automation' : task.command}`, 'error');
            ...
            <div style={{
                ...
                {/* Global Sidebars - Right Side Only */}
                {/* Settings Sidebar */}
                <div style={{
                    position: 'fixed',
                    right: showSettingsSidebar ? '24px' : '-420px',
                    top: '100px',
                    width: '380px',
                    ...
                    <div style={{ marginBottom: '12px' }}>
                        ...
                        <select value={csGoal} onChange={e => setCsGoal(e.target.value)} style={{ width: '100%', padding: '10px', background: theme === 'light' ? '#f9fafb' : 'rgba(255,255,255,0.05)', border: theme === 'light' ? '1px solid rgba(0,0,0,0.15)' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: theme === 'light' ? '#111827' : 'white', fontSize: '13px' }}>
                            ...
                            <option value="DifferentPerspective">Perspective</option>
                            <option value="BuildRelationship">Relationship</option>
                            <option value="SubtlePitch">Subtle Pitch</option>
                        </select>
                    </div>
                    ...
                    <div style={{ marginBottom: '12px' }}>
                        ...
                        <select value={csLength} onChange={e => setCsLength(e.target.value)} style={{ width: '100%', padding: '10px', background: theme === 'light' ? '#f9fafb' : 'rgba(255,255,255,0.05)', border: theme === 'light' ? '1px solid rgba(0,0,0,0.15)' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: theme === 'light' ? '#111827' : 'white', fontSize: '13px' }}>
                            ...
                            <option value="Long">Long (≤900)</option>
                        </select>
                    </div>
                    ...
                    <div style={{ marginBottom: '12px' }}>
                        ...
                        <select value={csStyle} onChange={e => setCsStyle(e.target.value)} style={{ width: '100%', padding: '10px', background: theme === 'light' ? '#f9fafb' : 'rgba(255,255,255,0.05)', border: theme === 'light' ? '1px solid rgba(0,0,0,0.15)' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: theme === 'light' ? '#111827' : 'white', fontSize: '13px' }}>
                            ...
                            <option value="challenger">Challenger (Different view)</option>
                            <option value="supporter">Supporter (Validate)</option>
                            <option value="expert">Expert (Data refs)</option>
                            <option value="conversational">Casual (Colleague-like)</option>
                        </select>
                    </div>
                ...
                {/* Sources Sidebar - Style Sources */}
                <div style={{
                    position: 'fixed',
                    right: showSourcesSidebar ? '24px' : '-380px',
                    top: '100px',
                    width: '340px',
                    maxHeight: 'calc(100vh - 140px)',
                    background: theme === 'light' ? '#ffffff' : 'rgba(17, 24, 39, 0.98)',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                        <h3 style={{ color: theme === 'light' ? '#111827' : 'white', fontSize: '16px', fontWeight: '700', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {commentStyleProfiles.length > 0 && (
                                <span style={{ background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', color: 'white', fontSize: '10px', fontWeight: '600', padding: '2px 8px', borderRadius: '10px', marginLeft: '4px' }}>
                                    {commentStyleProfiles.length}
                                </span>
                            )}
                        </h3>
                    ...
                    <div style={{ marginBottom: '12px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <button
                                ...
                                style={{ padding: '10px 16px', background: commentStyleScraping ? 'rgba(139,92,246,0.5)' : 'linear-gradient(135deg, #8b5cf6, #7c3aed)', border: 'none', borderRadius: '8px', color: 'white', fontWeight: '600', cursor: commentStyleScraping ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}
                            >
                    ...
                    <div style={{ marginTop: '8px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', padding: '8px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', cursor: 'pointer' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'white', fontSize: '13px', fontWeight: '600' }}>
                                ...
                                Saved Profiles
                                <span style={{ background: 'rgba(139,92,246,0.2)', color: '#a78bfa', fontSize: '10px', fontWeight: '600', padding: '2px 8px', borderRadius: '10px' }}>
                                    {commentStyleProfiles.length}
                                </span>
                            </div>
                        </div>
                        ...
                        {commentStyleProfiles.length === 0 ? (
                            <div style={{ padding: '24px 16px', textAlign: 'center' }}>
                                <div style={{ color: '#8b5cf6', marginBottom: '12px' }}>
                                    {miniIcon('M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z', '#693fe9', 32)}
                                </div>
                                <h4 style={{ color: 'white', fontSize: '14px', margin: '0 0 8px 0' }}>No Profiles Yet</h4>
                                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '12px', margin: 0 }}>Add a LinkedIn profile above to start.</p>
                            </div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '350px', overflowY: 'auto' }}>
                                {commentStyleProfiles.map((profile: any) => {
                                    ...
                                    const isExpanded = commentStyleExpanded === profile.id;
                                    return (
                                        <div key={profile.id}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: profile.isSelected ? 'rgba(139,92,246,0.1)' : 'rgba(255,255,255,0.03)', borderRadius: '8px', border: profile.isSelected ? '1px solid rgba(139,92,246,0.3)' : '1px solid transparent' }}>
                                                {/* Avatar with initials */}
                                                <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px', fontWeight: '700', flexShrink: 0 }}>
                                                ...
                                                {/* Train toggle */}
                                                <div onClick={() => toggleProfileSelect(profile.id)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                    <div style={{ width: '32px', height: '18px', borderRadius: '9px', background: profile.isSelected ? '#8b5cf6' : 'rgba(255,255,255,0.2)', position: 'relative', transition: 'all 0.2s' }}>
                                                        <div style={{ width: '14px', height: '14px', borderRadius: '50%', background: 'white', position: 'absolute', top: '2px', left: profile.isSelected ? '16px' : '2px', transition: 'all 0.2s' }} />
                                                    </div>
                                                    <span style={{ fontSize: '10px', color: profile.isSelected ? '#8b5cf6' : 'rgba(255,255,255,0.4)', fontWeight: '600', whiteSpace: 'nowrap' }}>Train</span>
                                                </div>

                                                {/* View Comments button (expand inline) */}
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    title="View comments inline"
                                                >
                                                    {isExpanded ? miniIcon('M19 9l-7 7-7-7', 'rgba(255,255,255,0.5)', 12) : miniIcon('M9 5l7 7-7 7', 'rgba(255,255,255,0.5)', 12)}
                                                </button>

                                                {/* View All Comments Popup button */}
                                                <button
                                                    onClick={async () => {
                                                        // Load all comments for this profile
                                                        const token = localStorage.getItem('authToken');
                                                        setCommentStyleCommentsLoading(true);
                                                        try {
                                                            const res = await fetch(`/api/scraped-comments?profileId=${profile.id}`, {
                                                                headers: { 'Authorization': `Bearer ${token}` }
                                                            });
                                                            const data = await res.json();
                                                            if (data.success) {
                                                                setProfileCommentsModal({ profile, comments: data.comments || [] });
                                                            } else {
                                                                showToast('Failed to load comments', 'error');
                                                            }
                                                        } catch (err) {
                                                    ...
                                                    title="View all comments"
                                                >
                                                    {miniIcon('M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z', 'rgba(255,255,255,0.5)', 14)}
                                                </button>
                ...
                {profileCommentsModal && (
                    <div style={{
                        ...
                        justifyContent: 'center',
                        zIndex: 1000,
                    }} onClick={() => setProfileCommentsModal(null)}>
                        <div style={{
                            ...
                            <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '700' }}>
                                        {(profileCommentsModal.profile.profileName || profileCommentsModal.profile.profileId || 'U').split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2)}
                                    </div>
                                    <div>
                                        <h3 style={{ color: 'white', fontSize: '16px', fontWeight: '600', margin: 0 }}>
                                            {profileCommentsModal.profile.profileName || profileCommentsModal.profile.profileId}
                                        </h3>
                                        <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '12px' }}>
                                            {profileCommentsModal.comments.length} comments
                                        </span>
                                    </div>
                                </div>
                                <button onClick={() => setProfileCommentsModal(null)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: '24px', lineHeight: 1 }}>×</button>
                            </div>

                            {/* Modal Content */}
                            <div style={{ padding: '16px 20px', overflowY: 'auto', flex: 1 }}>
                                {profileCommentsModal.comments.length === 0 ? (
                                    <div style={{ textAlign: 'center', padding: '40px 0', color: 'rgba(255,255,255,0.5)' }}>
                                        No comments found for this profile.
                                    </div>
                                ) : (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                        {profileCommentsModal.comments.map((comment: any) => (
                                            <div key={comment.id} style={{
                                                background: comment.isTopComment ? 'rgba(245,158,11,0.1)' : 'rgba(255,255,255,0.03)',
                                                padding: '14px',
                                                borderRadius: '10px',
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                                                    ...
                                                    {comment.isTopComment && (
                                                        <span style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '4px', background: 'rgba(245,158,11,0.2)', color: '#fbbf24', fontWeight: '600' }}>
                                                            Top Comment
                                                        </span>
                                                    )}
                                                </div>
                                                <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '13px', lineHeight: '1.5', margin: 0, whiteSpace: 'pre-wrap' }}>
                                                    {comment.commentText}
                                                </p>
                                            </div>
                                        ))}
                            ...
                            <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'flex-end' }}>
                                <button onClick={() => setProfileCommentsModal(null)} style={{ padding: '8px 20px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '8px', color: 'white', cursor: 'pointer', fontWeight: '500' }}>
                                    Close
                                </button>
                            </div>
                        </div>
